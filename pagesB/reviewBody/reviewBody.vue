  ...
  <view class="review-body">
      ...
      <uni-search-bar 
        ...
        <view class="is-select">
            ...
            <view class="box">
                <view class="item" v-for="(item, index) of reviewlist" :key="index">
                    {{ item.nickName }}
                    <view class="close">
        ...
        <view class="usage" v-show="current === 0">
            ...
            <view>
                    <view 
                       ...
                       <view class="checkbox">
                           <view 
							   v-if="reviewlist.indexOf(item) != -1"
							   class="checked">
						   </view>	
                        </view>
                    </view>
...
<script setup>
    ...
    //移除已选
    const remove = (obj) => {
		const index = reviewlist.value.findIndex(item => item.userId === obj.userId);
		if(index < 0) return;
		reviewlist.value.splice(index, 1);
    };
    
	const checkClick = (options) => {
		store.SET_REVIEWLIST(options);
	};
    // 父组件回调
